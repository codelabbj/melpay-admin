...
import {useChangePassword, UserProfile, useUpdateProfile} from "@/hooks/use-profile";
...
export default function ProfilePage() {
    ...
    const [user, setUser] = useState<ReturnType<typeof getUserData>>()
            ...
            updateUser.mutate(
                updateData,
                {
                    onSuccess : ()=>{
                        ...
                        setUserData({...user,last_name:data.last_name,email:data.email,first_name:data.first_name,phone:data.phone})
                        // Only include password if it's being changed
                        if (data.password && data.password.length > 0) {
                            changeUserPassword.mutate({new_password:data.password, confirm_new_password:data.confirm_password!, old_password : data.old_password!})
                        }
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 grid grid-cols-1 items-center justify-center mt-10">
